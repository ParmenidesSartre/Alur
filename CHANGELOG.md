...
### Fixed
- **EventBridge Generation Bug** - Fixed `AttributeError` when generating `eventbridge.tf`
  - Error: `'InfrastructureGenerator' object has no attribute 'env'`
  - Now correctly reads environment from `config.settings.ENVIRONMENT` or defaults to 'dev'
  - EventBridge rules now generate successfully with scheduled pipelines
  - Resolves issue preventing `alur deploy` from working with `@schedule` decorator
## [0.7.3] - 2026-01-23

### Fixed
- **Windows CLI Compatibility** - Fixed Unicode encoding error in `alur schedules` command
  - Replaced Unicode checkmarks (✓/✗) with ASCII text
  - Command now works correctly on Windows CMD and PowerShell
  - Resolves: `UnicodeEncodeError: 'charmap' codec can't encode character`
## [0.7.2] - 2026-01-23
